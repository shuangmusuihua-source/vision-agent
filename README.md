...
Tag Release 通过 GitHub Actions 构建、验证并上传发布草稿。配置 Developer ID 证书后，使用 electron-builder 内置流程完成 hardened runtime、签名和 notarization；未配置 `CSC_LINK` 时，工作流禁用证书自动发现并生成未签名、未公证的安装包，正式发布时应在说明中注明。签名使用 `CSC_LINK`、`CSC_KEY_PASSWORD`；notarization 使用 `APPLE_API_KEY`、`APPLE_API_KEY_ID`、`APPLE_API_ISSUER`，或本地 Apple ID 方式的 `APPLE_ID`、`APPLE_APP_SPECIFIC_PASSWORD`、`APPLE_TEAM_ID`。仓库本地缺少发布证书时，`npm run pack` 仍会生成未签名的验证包；完整变量见 `.env.example`。
