...
import { parseFrontmatter, buildPage } from "./frontmatter";
...
Output the merged complete page.`;
  ...
  // Invalid output must never replace the old page with only the new material.
  const mergedParsed = parseFrontmatter(out);
  if (!out.trim() || !mergedParsed.hasFrontmatter || !mergedParsed.body.trim()) {
    throw new Error('主题合并结果不完整，原内容已保留，请重试');
  }
